...
def trainset_feature_scaling(trainset: pd.DataFrame, minmax=False):
    ...
    categorical_trainset = trainset[
        ["Telefono", "Deceduto", "CittadinanzaItaliana", "Estero", "NuovoContribuente", "label"]]
    trainset.drop(columns=["Telefono", "Deceduto", "CittadinanzaItaliana", "Estero", "NuovoContribuente", "label"],
                  inplace=True)
    if minmax == False:
        # StandardScaler
...
# trainset: 18560 totali, 3403 label 1, 15157 label 0
# testset: 4903 totali, 973 label 1, 3930 label 0

# Load Dataset
trainset: pd.DataFrame = load_raw_data("datasets/export_trainset_PF.csv")  # 18557 entry
testset: pd.DataFrame = load_raw_data("datasets/export_testset_PF.csv")  # 4903 entry
...
bestforest = RandomForestClassifier(bootstrap=True, ccp_alpha=0.0, class_weight='balanced',
